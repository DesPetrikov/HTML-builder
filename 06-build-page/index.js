...
const destinationPathFonts = path.join(__dirname, 'project-dist','assets', 'fonts');

const currentPathImg = path.join(__dirname, 'assets', 'img');
const destinationPathImg = path.join(__dirname, 'project-dist','assets', 'img');

const currentPathSvg = path.join(__dirname, 'assets', 'svg');
const destinationPathSvg = path.join(__dirname, 'project-dist','assets', 'svg');
...
async function readHtml(){
	...
	const readTemplateStream = fs.createReadStream(path.join(__dirname, 'template.html'), {encoding: 'utf-8'});
	let bundleData = '';
	...
	const dir = await fs.promises.readdir(componentshPath, {withFileTypes: true});
	for await(let entry of dir){
		if(entry.isFile() && path.extname(entry.name) === '.html'){
			const readStream = fs.createReadStream(path.join(__dirname, 'components', entry.name), {encoding: 'utf-8'});
			
			const newPromise = () => new Promise((resolve) => {
				readStream.on('data', (chunk) => {
					let data ='';
					data += chunk;
					const regExp = new RegExp('{{' + path.basename(path.join(__dirname, 'components', entry.name), path.extname(entry.name)) + '}}')
...
async function copyDir (currentPath, destinationPath, folderType){
	try {
		...
		for await (let entry of dir){
			if(entry.isFile()){
				await fs.promises.copyFile((path.join(__dirname, 'assets', folderType, entry.name) ),(path.join(__dirname, 'project-dist', 'assets', folderType, entry.name)));
			}
		}
	} catch {
